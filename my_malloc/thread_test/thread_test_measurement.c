...
char dumb_hash(void* p) {
  unsigned char h = (unsigned char) (17*(uintptr_t)p*(uintptr_t)p + 7*(uintptr_t)p + 3*((uintptr_t)p>>8)); // simple but weak hash
  return h;
}
...
void do_allocate(int thread_id) {
  ...
  for (i=0; i < NUM_ITEMS; i++) {
    ...
    malloc_items[index].address = (int *)MALLOC_WITH_CHECK(malloc_items[index].bytes);
	...
	if (do_free == 1) {
	  FREE_WITH_CHECK(malloc_items[counter].address,malloc_items[counter].bytes);
	  counter++;
	} //if
      } //if
    }
